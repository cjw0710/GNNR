...
class GraphSAGE(MessagePassing):
    def __init__(self, in_channels: int, out_channels: int, aggr: str = 'mean', **kwargs):
        super(GraphSAGE, self).__init__(aggr=aggr, **kwargs)

        self.lin = Linear(in_channels + out_channels, out_channels)
    ...
    def update(self, aggr_out, x):
        return self.lin(torch.cat([aggr_out, x], dim=-1))    
...
#         self.sage_s1 = GraphSAGE(num_features, num_hidden)
#         self.sage_s2 = GraphSAGE(num_features, num_hidden)
#         self.sage_f1 = GraphSAGE(num_features, num_hidden)
#         self.sage_f2 = GraphSAGE(num_features, num_hidden)
#         self.l1 = l1
#         self.l2 = l2
...
#     def reset_parameters(self):
#         self.sage_s1.reset_parameters()
#         self.sage_s2.reset_parameters()
#         self.sage_f1.reset_parameters()
#         self.sage_f2.reset_parameters()

#     def forward(self, x: torch.Tensor,
#                 edge_index: torch.Tensor, knn_graph: torch.Tensor) -> torch.Tensor:
#         h0 = self.sage_s1(x, edge_index) 
#         h1 = self.sage_s1(x, edge_index)
#         z0 = self.sage_f2(x, edge_index)
#         z1 = self.sage_f2(x, edge_index) + self.sage_f2(x, knn_graph)
#         return h0, h1, z0, z1
...
class Model(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor,
        ...
        h1 = self.gcn_s1(x, edge_index, K=10)
