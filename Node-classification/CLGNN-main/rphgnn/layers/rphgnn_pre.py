...
def rphgnn_propagate_then_update_cora(g, current_k, inner_k, input_x_dim_dict, target_node_type, squash_strategy, norm=None, squash_even_odd="all", squash_self=True, collect_even_odd="all", diag_dict=None, train_label_feat=None):
...
def rphgnn_propagate_and_collect_cora(g, k, inner_k, alpha, target_node_type, use_input_features, squash_strategy, train_label_feat, norm, squash_even_odd, collect_even_odd, squash_self=False, target_feat_random_project_size=None, add_self_group=False):
    with torch.no_grad():
        raw_input_target_x = g.ndata["feat"]
        with g.local_scope():
            embedding_size = g.ndata["feat"].size(-1)
            print(embedding_size) 
            diag_dict = None
            input_x_dim_dict = {
                "x_dim": g.ndata["feat"].size(-1)
            }
            input_x_dict = {
                "paper": g.ndata["feat"]
            }
            input_target_x = g.ndata["feat"]
            target_h_list_list = []
            for k_ in range(k):
                ...
                (target_h_list_list_, target_sorted_keys), g = rphgnn_propagate_then_update_cora(g, k_, inner_k, input_x_dim_dict, target_node_type, squash_strategy=squash_strategy, norm=norm, squash_even_odd=squash_even_odd, collect_even_odd=collect_even_odd, squash_self=squash_self, diag_dict=diag_dict, train_label_feat=train_label_feat)
               
                target_h_list_list.extend(target_h_list_list_)

                g.ndata["feat"] = g.ndata["feat"] * (1 - alpha) + input_x_dict["paper"] * alpha
