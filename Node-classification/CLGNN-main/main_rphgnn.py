...
if args.dataset == "cora":
    ...
    y = torch.load("./data/cornell_labels.pt", map_location=torch.device('cpu')).detach().cpu().numpy()
elif args.dataset == "texas":
    y = torch.load("./data/texas_labels.pt", map_location=torch.device('cpu')).detach().cpu().numpy()
    print("加载的标签顺序:", y)
    ...
    if running_leaderboard_mag:
        ...
        with torch.no_grad():
            with torch.autocast(device_type=device): #dtype=self.autocast_dtype
                ...
                print("预测的类别：", batch_y_pred.detach().cpu().numpy())
                print("真实的标签：", batch_y.detach().cpu().numpy())
                print(test_index)
